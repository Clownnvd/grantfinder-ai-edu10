...
health=client.get('/health');assert health.status_code==200 and health.json()['open_opportunities']==968
matched=client.post('/api/v1/match',json={'role':'researcher','profile':profile,'top_k':3});assert matched.status_code==200
body=matched.json();assert len(body['top_matches'])==3 and body['tool_trace'][-1]['status']=='waiting_for_human'
